...
namespace FirstWebApiApplication.Service
{
    public class UserService
    {
        ...
        public Response CreateUser(UserDTO inputRequest)
        {
        ...
        public Response GetUserDetails(UserDTO user)
        {
            var genricUserObj = new SchoolRepository<User>();
            var userList = genricUserObj.ReadUser(@".\Json\User.json");
            var flag = userList.FirstOrDefault(item => item.UserId == user.UserId);
            if (flag != null)
            {
                throw new Exception("Already exist id");
            }
            
            var genricUserDetailsObj = new SchoolRepository<UserDetails>();
            var userDetailsList = genricUserDetailsObj.ReadUser(@".\Json\UserDetails.json");
            var userId = userDetailsList.Count > 0 ? userDetailsList[userDetailsList.Count - 1].Id + 1 : 0;
            userList.Add(new User()
            {
            ...
            userDetailsList.Add(new UserDetails()
            {
                ...
                IsEmployee = user.IsStudent,
                IsActive = true,
            });
            genricUserObj.SaveUser(@".\Json\User.json", userList);
            genricUserDetailsObj.SaveUser(@".\Json\UserDetails.json", userDetailsList);
